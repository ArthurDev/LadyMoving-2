...
<div id="primary_block" class="row block clearfix">
	...
	<div id="pb-left-column" class="col-md-6 col-sm-6">
		...
		<div class="content_prices clearfix">
			...
			<p id="add_to_cart" {if (!$allow_oosp && $product->quantity <= 0) OR !$product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE}style="display:none"{/if} class="buttons_bottom_block btn">
...
{if (isset($product) && $product->description) || (isset($product) && $product->mode_emploi) || (isset($features) && $features) || (isset($accessories) && $accessories) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || isset($product) && $product->customizable}
<div id="more_info_block" class="block clear">
	<ul id="more_info_tabs" class="idTabs idTabsShort clearfix">
		{if $product->description}<li><a id="more_info_tab_more_info" href="#idTab1">{l s='More info'}</a></li>{/if}
		<!-- Ajout d'un onglet "Mode d'emploi" -Arthur- -->
		{if $product->mode_emploi}<li><a id="more_info_tab_more_info" href="#idTab3">{l s='Mode d\'emploi'}</a></li>{/if}
	...
	{/if}
	{if isset($product) && $product->mode_emploi}
		<!-- mode d'emploi -->
		<div id="idTab3" class="rte">{$product->mode_emploi}</div>
	{/if}
