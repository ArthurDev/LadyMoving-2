...
class AdminCustomersControllerCore extends AdminController
{
	...
	public function __construct()
	{
		...
		// Récupère les noms des employés à partir des id -Arthur-
		/*
		$list_employes = $this->context->customer->id_employee;

		$total_employee = count($list_employes);
		for ($i = 0; $i < $total_employee; $i++)
		{
			$groupEmploye = new Employee($total_employee[$i]);
			$list_employes[$i] = array();
			$list_employes[$i]['id_employee'] = $groupEmploye->id;
			$list_employes[$i]['firstname'] = $groupEmploye->firstname[$this->default_form_language];
		}*/
		...
		//-Arthur-		
		/*// Selection du champ id_employee dans la table customer
		$this->_select .= ', a.id_employee AS id_employee, cl.employee AS employee';
		// Jointure vers la table id_employee de la table Employee
		$this->_join .= 'LEFT JOIN`'._DB_PREFIX_.'employee` ad ON (ad.`id_employee` = a.`id_employee`)';

		$employees_array = array();
		$employees = Employee::getEmployees((int)$this->context->employee->firstname);

		foreach ($employees as $employee)
				$employees_array[$employee['id_employee']] = $employee['firstname'];*/
		...
		$this->fields_list = array(
			...
			// Affiche le nom de l'employé ayant créé le compte client -Arthur-
			/*'id_employee' => array(
	...
	public function renderForm()
	{
		...
		$list_employes = Employee::getEmployees();
		...
		$this->fields_form = array(
			...
			'input' => array(
				// Création de la liste pour le choix de l'employé -Arthur-
				array(
					'type' => 'select',
					'label' => $this->l('Employé'),
					'name' => 'id_employee',
					'options' => array(
						'query' => $list_employes,
						'id' => 'id_employee',
