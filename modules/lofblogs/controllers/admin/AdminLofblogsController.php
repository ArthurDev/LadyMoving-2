...
class AdminLofblogsController extends AdminController {
    ...
    protected function copyFromPost(&$object, $table) {
        ...
        if(!Tools::getIsset('access'))
            $_POST['access'] = 0;
        if (is_array(Tools::getValue('access'))) {
            $_POST['access'] = trim(implode(',', Tools::getValue('access')), ' ,');
        }

        //update feature state :
        if (!Tools::getValue('featured'))
            $_POST['featured'] = 0;
    ...
    function generateDefaultValuesLang() {
        global $cookie;
        $defaultTitle = Tools::getValue('title_' . $cookie->id_lang);
        $languages = Language::getLanguages(false);
        foreach ($languages as $lang) {

            //prepare data :
            $title = Tools::getValue('title_' . $lang['id_lang']) ? Tools::getValue('title_' . $lang['id_lang']) : $defaultTitle;
            $alias = (Tools::getValue('link_rewrite_' . $lang['id_lang']) ? Tools::getValue('link_rewrite_' . $lang['id_lang']) : $defaultAlias);
            ...
            if ($title) {
				...
				*/
				$_POST['link_rewrite_' . $lang['id_lang']] = $alias;
                ...
                //meta description
                if (!Tools::getValue($meta_desc))
                    $_POST[$meta_desc] = $title;

                //meta keyworks :
                if (!Tools::getValue($meta_key))
    ...
    public function initProcess() {
        parent::initProcess();
        if ((Tools::getIsset('featured' . $this->table) || Tools::getIsset('featured')) && Tools::getValue($this->identifier)) {
