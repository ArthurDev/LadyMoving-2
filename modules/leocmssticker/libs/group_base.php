...
if( !class_exists('LofDataSourceBase', false) ){ 
	 /**
 ...
 abstract class LofDataSourceBase{
    ...
    public function generateImages( $item, $params ){        
		...
		if($params->get("cre_main_size",0)){
			...
			if($params->get("main_width_theme",0)){
				$main_img_size["width"]  = $params->get("main_width_theme",556);	
			}else{
				$main_img_size["width"]  = $params->get("main_width",556);	
			}            
            $item["mainImge"] = $this->resizeImage( $item["mainImge"], $main_img_size ); 
		}
        if($params->get("aurenthumb",1)){
    ...
    */
	...
	static function substring( $text, $length = 100, $replacer = "..."){
